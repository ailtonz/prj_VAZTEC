    ...
    Select Case Operacao
     ...
     Case "Excluir"
        ...
        If MsgBox("Deseja excluir este registro?", vbInformation + vbOKCancel) = vbOK Then
           ...
           DoCmd.RunSQL ("Delete " & rstFormularios.Fields("Identificacao") & " from " & strTabela & " where " & rstFormularios.Fields("Identificacao") & " = " & Form_Pesquisar.lstCadastro.Value)
...
Err_Manipulacao:
    If Err.Number = 3086 Then
        DoCmd.SetWarnings False
        MsgBox "Você não pode excluir este registro."
        DoCmd.SetWarnings True
    End If
    MsgBox Err.Description
    Resume Exit_Manipulacao
